...
export const schema = yup.object().shape({
  ...
  email: yup.string().required(reqMsg).min(3, "Niepoprawny adres e-mail.").max(50, "Zbyt wiele znaków.").email("Niepoprawny adres e-mail"),
...
//   postalCode: yup.string().required(reqMsg).max(7, "Zbyt wiele znaków."),
  msg: yup.string().required(reqMsg).max(500, "Zbyt wiele znaków."),
});
